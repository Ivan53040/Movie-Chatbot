...
Write-Host "Starting LangChain chatbot server on $url ..." -ForegroundColor Cyan
$serverCommand = "cd /d `"$projectRoot`" && python -m uvicorn fastapi_server_langchain:app --host 127.0.0.1 --port $port"
Start-Process cmd.exe -ArgumentList "/k", $serverCommand -WorkingDirectory $projectRoot | Out-Null

Start-Sleep -Seconds 3
Start-Process $url | Out-Null

Write-Host "LangChain chatbot opened at $url" -ForegroundColor Green
